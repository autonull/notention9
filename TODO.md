...
- [x] **Verification**:
  - [x] Write `core/tests/matching.test.ts`:
    - Test all operators with various types
    - Test partial matching scoring
    - Test performance with 1k, 10k notes
  - [x] Manual: Create request + offer notes in UI, verify correct matches shown
...
- [x] Property parsing improvements:
...
- [x] **Verification**:
  - [x] Write `core/tests/properties.test.ts`:
    - Parse all formats, verify correct AST
    - Test validation rules, macro expansion
  - [x] Integration: Type properties in UI, verify live validation
...
- [x] Ontology visualization (Developer Mode):
...
- [x] **Verification**:
  - [x] Write `core/tests/ontology.test.ts`:
    - Add notes, verify keys learned
    - Test type inference, export/import cycle
  - [x] Manual: Create 20 notes, check ontology graph shows patterns
...
- [x] **Verification**:
  - [x] Set up local Nostr relay (nostream/strfry) for testing
  - [x] Write `verification/verify_nostr_publishing.ts`:
  ...
  - [x] Manual: Two instances, publish/subscribe, verify match notification
...
- [x] Hash-based intent broadcasting:
...
- [x] **Verification**:
  - [x] Write `verification/verify_distributed_coordination.ts`
  - [x] Run on CI with Docker Compose orchestration
  - [x] Generate report: graphs, metrics, pass/fail thresholds
...
- [x] Create `McpToolRegistry` class:
...
- [x] Create `McpTransportManager`:
  - Reusable SSE/session lifecycle management
  - Eliminate 25+ lines of duplication
  - Session metrics and graceful shutdown
- [x] **Verification**:
  - [x] Existing test: `npx tsx verification/verify_cli_mcp.ts` still passes
  - [x] Manual: Start server, connect CLI, verify both endpoints work
...
- [x] **Verification**:
  - [x] Write `verification/verify_plugins.ts`:
...
- [x] `ConfigManager` with hierarchical merging:
...
- [x] **Verification**:
  - [x] Write `verification/verify_config.ts`:
...
- [x] **Verification**:
  - [x] Write `verification/verify_semantic_understanding.ts`:
...
- [x] **Verification**:
  - [x] Write `verification/verify_llm_streaming.ts`:
...
- [x] **Verification**:
  - [x] Manual walkthrough video
  - [x] User testing: 3 new users, collect feedback
...
- [x] Simulation scenario DSL:
  ...
  ```
- [x] Agent behavior profiles:
  - Rational (best match), Random (exploration), Realistic (probabilistic)
- [x] Metrics:
  - Match rate, response latency, ontology convergence
- [x] **Verification**:
  - [x] Refactor existing `verify_cli_mcp.ts` to use DSL
  - [x] Add scenarios from `use-cases.md`
  - [x] Run with 10, 50, 100 agents, measure scaling
