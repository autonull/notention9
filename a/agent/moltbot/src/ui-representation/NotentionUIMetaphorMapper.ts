...
export class NotentionUIMetaphorMapper implements UIMetaphorMapper {
  private metaphors: UIMetaphor[] = [
    {
  ...
  mapToMetaphor(clawdBotConcept: any): UIMetaphor {
    ...
    const matchedMetaphor = this.metaphors.find(m =>
      m.category === type || m.id.includes(type.toLowerCase())
    );

    if (matchedMetaphor) {
      return matchedMetaphor;
    }

    // Default to conditional automation for most cases
    return this.metaphors[0];
  }
  ...
  mapFromMetaphor(metaphor: UIMetaphor): any {
    ...
    return {
      ...
      template: metaphor.template,
      properties: metaphor.properties,
      settings: {
        uiMetaphor: metaphor.id,
        icon: metaphor.icon,
