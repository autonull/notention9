...
export function MobileNavigation() {
    ...
    const {notificationCount, chatNotificationCount} = useView();
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-gray-800 z-50 pb-safe">
            <div className="flex justify-around items-center h-16 px-2">
                {mobileNavItems.map((item) => {
                    const badgeCount = item.badgeCountKey === 'notificationCount' ? notificationCount :
                                      (item.badgeCountKey === 'chatNotificationCount' ? chatNotificationCount : 0);
                    return (
                        <button
                            key={item.id}
                        ...
                        >
                            ...
                            {badgeCount > 0 && (
                                <Badge variant="danger" size="sm" pill className="absolute top-2 right-1/4 h-4 min-w-[16px] font-bold">
