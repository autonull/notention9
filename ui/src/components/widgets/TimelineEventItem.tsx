...
import { NoteIcon, SparklesIcon, NetworkIcon, CpuChipIcon } from '../common/icons';
...
export function TimelineEventItem({ event }: TimelineEventItemProps) {
  ...
  return (
    <div className="relative">
      {/* Dot on timeline */}
      <div className={`absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full border ${getColor()} bg-gray-900`}></div>

      <div className="flex flex-col gap-1">
        <div className="flex justify-between items-start">
          <span className="text-sm text-gray-300 font-medium">{event.description}</span>
          <span className="text-xs text-gray-500">{event.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
        </div>
        {event.metadata && (
