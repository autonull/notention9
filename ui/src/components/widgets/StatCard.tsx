...
export function StatCard({ label, value, icon, trend, trendValue, className = '' }: StatCardProps) {
  return (
    <div
      className={`bg-gray-800 p-4 rounded-xl border border-gray-700/50 shadow-sm ${className}`}
      role="region"
    ...
    >
      <div className="flex justify-between items-start mb-2">
        ...
        {icon && <div className="text-gray-500 opacity-80" aria-hidden="true">{icon}</div>}
      </div>
