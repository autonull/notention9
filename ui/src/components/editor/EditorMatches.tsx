...
import {convertEventToNote, parseProperties} from '@notention/core';
import {Badge} from '../common/Badge';
import {ChatIcon, CheckCircleIcon, ChevronDownIcon, PlusIcon, SearchSparkleIcon, XCircleIcon} from '../common/icons';
        ...
        <div className="border-t border-gray-700 bg-gray-900/50 flex flex-col animate-fade-in">
            ...
            <div className="p-3 space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar">
                {matches.map(({event, score, satisfied, failed}) => (
                    <div key={event.id}
                         className="bg-gray-800 p-3 rounded-lg border border-gray-700 hover:border-purple-500/50 transition-colors group">
                        <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center gap-2">
                                <div
                                    className={`flex items-center justify-center w-8 h-8 rounded-full ${score > 0.8 ? 'bg-green-900/50 text-green-400' : 'bg-purple-900/50 text-purple-400'}`}>
                        ...
                        <details className="group/details">
                            <summary
                                className="flex items-center gap-1 text-[10px] uppercase font-bold text-gray-500 cursor-pointer hover:text-gray-300 mb-2 list-none">
                                <ChevronDownIcon
                            ...
                            <div className="space-y-2 pl-2 mb-2 animate-fade-in">
                                {satisfied && satisfied.length > 0 && (
                                    <div className="space-y-1">
                                        <div
                                            className="text-[10px] font-semibold text-green-400/80 flex items-center gap-1">
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                            {satisfied.map((p: any, idx: number) => (
                                                <span key={`${p.key}-${idx}`}
                                                      className="text-[10px] px-1.5 py-0.5 rounded bg-green-900/30 text-green-300 border border-green-900/50 flex items-center gap-1">
                                                     <span className="font-mono">{p.key}</span>
                                                     <span
                                                         className="opacity-60 text-[9px]">{p.operator === 'is' ? ':' : p.operator} {p.values.join(', ')}</span>
                                                 </span>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                            {failed.map((p: any, idx: number) => (
                                                <span key={`${p.key}-${idx}`}
                                                      className="text-[10px] px-1.5 py-0.5 rounded bg-red-900/20 text-red-300 border border-red-900/30 opacity-80 flex items-center gap-1">
                                                     <span className="font-mono">{p.key}</span>
                                                     <span
                                                         className="opacity-60 text-[9px]">{p.operator === 'is' ? ':' : p.operator} {p.values.join(', ')}</span>
                                                 </span>
