...
export const SmartNoteAssistant: React.FC<SmartNoteAssistantProps> = ({ 
  ...
  const getIconForType = (type: string) => {
      switch (type) {
          case 'property': return <PlusIcon className="w-4 h-4 text-green-400" />;
          case 'link': return <LinkIcon className="w-4 h-4 text-blue-400" />;
          default: return <InformationCircleIcon className="w-4 h-4 text-purple-400" />;
      }
  };
              ...
              <div className="flex-1 min-w-0">
                  ...
                  {suggestion.confidence && (
                      <div className="flex items-center gap-1 mt-0.5">
                          ...
                          <span className="text-[10px] text-gray-500">{Math.round(suggestion.confidence * 100)}% match</span>
                      </div>
                  )}
              </div>
              <div className={`transition-opacity ${index === activeSuggestion ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
