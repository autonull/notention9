        ...
        <>
            ...
            <div className="h-1/3 overflow-y-auto p-2 font-mono text-[10px] space-y-1 bg-gray-900">
                {newAttributes.length === 0 && (
                    <div className="flex items-center justify-center h-full text-gray-600 italic">No new attributes
                        detected.</div>
                )}
                {newAttributes.map((attr, i) => (
                    <div key={i}
                         className="text-green-400 flex items-center gap-2 p-1.5 hover:bg-white/5 rounded transition-colors border border-transparent hover:border-green-500/20">
                        <span>🌱</span>
