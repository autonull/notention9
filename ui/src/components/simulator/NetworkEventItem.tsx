        ...
        >
            <div className="flex justify-between items-start mb-1.5">
                <div className="text-[9px] text-gray-500 font-mono">
                    {note.updatedAt ? new Date(note.updatedAt).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                        second: '2-digit'
                    }) : 'Now'}
                ...
                {isMatch && <span
                    className="text-[8px] bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 px-1 rounded animate-pulse font-bold">MATCH</span>}
            ...
            {note.tags.length > 0 && (
                <div className="flex flex-wrap gap-1">
                    {note.tags.map(tag => (
                        <span key={tag}
                              className="text-[9px] text-blue-400/70 hover:text-blue-400 transition-colors">#{tag}</span>
                    ))}
                </div>
            )}
