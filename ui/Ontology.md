...
In Notention, the **Ontology** is the shared language of the network. Unlike traditional apps with fixed databases, Notention's schema is **Emergent**.
...
-   **Matching Cycles:** Agents try to trade. If they fail (e.g., one uses `cost`, one uses `price`), the match score is low.
...
This simple tagging scheme allows any Nostr client to parse and index semantic data without needing to understand the full application state.
