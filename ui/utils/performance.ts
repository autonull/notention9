...
class PerformanceMonitor {
  ...
  measure(startMark: string, endMark: string, name?: string): number | null {
    ...
    if (startTime === undefined || endTime === undefined) {
      Logger.getInstance().warn(`Missing marks for measurement: ${startMark} -> ${endMark}`);
      return null;
    }
    ...
    Logger.getInstance().debug(`Performance: ${measureName} took ${duration.toFixed(2)}ms`);
      ...
      const observer = new PerformanceObserver((list) => {
        list.getEntries().forEach((entry) => {
          if (entry.duration > 50) {
            Logger.getInstance().warn(`Long task detected: ${entry.name} took ${entry.duration.toFixed(2)}ms`);
          }
        });
      });
  ...
  monitorLayoutThrashing(): void {
    if ('PerformanceObserver' in window) {
      const observer = new PerformanceObserver((list) => {
        list.getEntries().forEach((entry) => {
          Logger.getInstance().debug(`Layout shift: ${entry.name} caused ${entry.duration.toFixed(2)}ms`);
        });
      });
...
export function usePerformanceMonitor(componentName: string, deps: any[]): void {
  ...
  useEffect(() => {
    ...
    if (renders > 10) {
      Logger.getInstance().warn(`${componentName} has re-rendered ${renders} times, consider optimizing`);
    }
    ...
    if (changedDeps.length > 0 && renders > 1) {
      Logger.getInstance().debug(`${componentName} re-rendered due to changed deps:`, changedDeps);
    }
