...
import { TagIcon, ExclamationTriangleIcon } from '../common/icons';
...
export const PropertyChip = (props: NodeViewProps) => {
  ...
  return (
    ...
    >
      ...
      <span className={`${isValid ? 'text-blue-500' : 'text-red-500'} text-xs uppercase font-bold`}>{operator}</span>
      <span className={`font-mono px-1 rounded ${isValid ? 'bg-blue-900/50 text-blue-200' : 'bg-red-900/50 text-red-200'}`}>{value}</span>
