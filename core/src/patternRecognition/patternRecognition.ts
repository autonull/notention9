...
import { Pattern, UserBehaviorPattern, Prediction, PredictionResult } from './types.js';
import {
...
const DEFAULT_CONFIG: PatternRecognitionConfig = {
	coOccurrence: { minOccurrences: 2, temporalWindowMs: 60 * 60 * 1000, minConfidence: 0.3 },
	sequential: { maxTimeDiffMs: 24 * 60 * 60 * 1000, defaultConfidence: 0.6, minAccuracy: 0.3 }
};
...
export class PatternRecognitionService extends BaseService {
	...
	analyzeUserPatterns(userId: string, notes: Note[]): UserBehaviorPattern {
		return this.safeExecuteSync(() => {
			...
			const newPatterns = this.discoverPatterns(propertyFrequency, temporalPatterns, notes);
	...
	private discoverPatterns(
		...
		const coOccurrencePatterns = discoverCoOccurrencePatterns(propertyFrequency, temporalPatterns, this.config.coOccurrence);
	...
	recordPredictionOutcome(predictionId: string, wasAccurate: boolean, feedback?: string): void {
		this.safeExecuteSync(() => {
			...
			if (prediction) {
				...
				logInfo(`Prediction outcome recorded`, { predictionId, wasAccurate, feedback: !!feedback, serviceId: this.id });
			}
	...
	getUserPredictionStats(userId: string) {
		return this.safeExecuteSync(() => {
			...
			if (!userPatterns) return { totalPredictions: 0, accuracyRate: 0, patternsCount: 0 };
			...
			return { totalPredictions, accuracyRate: safeDivide(totalAccuracy, totalPredictions), patternsCount: patterns.length };
